...
export default function App() {
  ...
  return (
    <div className="app-shell">
          ...
          <AnimatePresence mode="wait">
            {view === 'dashboard' && (
              <motion.div key="dashboard" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.3 }}>
                <Dashboard niches={niches} loading={loading} onSelectNiche={(n) => { setSelectedNiche(n); setView('explore'); }} />
              </motion.div>
            )}
            {view === 'explore' && (
              <motion.div key="explore" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.3 }}>
                <NicheGrid niches={niches} loading={loading} onSelect={setSelectedNiche} />
              </motion.div>
            )}
            {view === 'analytics' && (
      ...
      <AnimatePresence>
        {selectedNiche && (
          <NicheDetail niche={selectedNiche} onClose={() => setSelectedNiche(null)} onDelete={handleDeleteNiche} />
        )}
      </AnimatePresence>
